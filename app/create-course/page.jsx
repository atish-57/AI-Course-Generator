...
const CreateCourse = () => {
  ...
  const GenerateCourseLayout = async () => {
    ...
    try {
      ...
      setLoading(false);
      alert("Failed to parse AI response as JSON. Please try again or check the prompt.");
      return;
    }
    setLoading(false);
    saveCourseLayoutDb(parsedResult);
  };
          ...
          {StepperOptions.map((item, index) => (
            <div key={index} className="flex items-center">
              <div className="flex flex-col items-center w-[50px] md:w-[100px]">
                <div
                  className={`bg-gray-300 p-3 rounded-full text-white ${activeIndex >= index && "bg-purple-500"
                    }`}
                >
                  {item.icon}
                </div>
              ...
              {index != StepperOptions.length - 1 && (
                <div
                  className={`h-1 w-[50px] md:w-[100px] rounded-full bg-gray-300 lg:w-[170px] ${activeIndex - 1 >= index && "bg-purple-500"
                    } `}
                ></div>
              )}
            </div>
